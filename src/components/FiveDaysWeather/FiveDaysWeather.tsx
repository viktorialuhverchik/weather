    ...
    return (
        <div className="card-container">
            ...
            <Card className="fivedays-weather_card">
                {!Object.keys(fiveDaysWeather).length ? <Alert severity="error"  data-testid="error">Weather in {city} not found!</Alert> : (Object.values(fiveDaysWeather).map((item: any, index: number) => {
                    return (
                        <div className="day-weather_container" key={index}>
                            <Card className="day-weather_card">
                                ...
                                <img
                                    src={!item.weather ? "" : `http://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`} data-testid="image"
